...
For general info on file formats (BED, narrowPeak, broadPeak) see UCSC Genome Browser documentation, specifically there are [3 ENCODE histone datasets](http://hgdownload.soe.ucsc.edu/goldenPath/hg19/encodeDCC/):  
...
Peak files are also available for all three datasets in [.bb format on UCSC](ttp://ftp.ebi.ac.uk/pub/databases/ensembl/encode/integration_data_jan2011/byDataType/peaks/jan2011/histone_macs/optimal/hub/).
  - The latest version of `rtracklayer` (>=1.5) has `import.bb` function, but can seem to install it...   
...
broad_ucsc_url <- "http://hgdownload.soe.ucsc.edu/goldenPath/hg19/encodeDCC"
ENCODE_urls <- list(BroadHistone=list(
  
))
list(ENCODE.BroadHistone.= file.path(broad_ucsc_url,"wgEncodeBroadHistone",
                        "wgEncodeBroadHistoneK562H3k27me3StdPk.broadPeak.gz")
     )
...
CT.bw_filt <- import.bw.parallel(bw.file = file.path(root.dir,"processed_data/HK5M2BBXY/bwa/mergedReplicate/bigwig/control.mRp.clN.bigWig"),
                          gr.dat = ENCODE.broadPeaks)
```
...
CT.summits <- rtracklayer::import(file.path(root.dir,peaks_dir,"control.mRp.clN_summits.bed")) 
CT.summits$width <- GenomicRanges::width(CT.summits)

summary(CT.summits$width)
```

## Import CUT&TAG peaks  
...
ChIPseeker::plotAnnoBar(CT.annotatePeaks)
```
...
```{r .narrowPeaks}
CT.narrowPeaks <- rtracklayer::import(file.path(root.dir,peaks_dir,"control.mRp.clN_peaks.narrowPeak")) 
...
CT.narrowPeaks_top$score <- GenomicRanges::score(CT.narrowPeaks_top_stored)
CT.narrowPeaks_top$width <- GenomicRanges::width(CT.narrowPeaks_top_stored)
...
```{r Compare overlap}
hits <- GenomicRanges::findOverlaps(query = CT.narrowPeaks, 
                                    subject = ENCODE.broadPeaks)
CT.narrowPeaks[S4Vectors::subjectHits(hits)]
```
...
CT.UCSC_path <- file.path(root.dir,peaks_dir,"control.mRp.clN_peaks.UCSC.narrowPeak")
