...
new_paths <- lapply(resources, function(x){
  path <- file.path(file.path(root.dir,"web"),x)
  # print(paste(x,"==>",path))
  dir.create(dirname(path), showWarnings = F, recursive = T)
  # Some html files have their resources saved separately. Copy those too.
  recursive <- basename(dirname(path))=="html"
  path_or_folder <- if(recursive) dirname(path) else path
  path_or_folder_x <- if(recursive) dirname(x) else x
  file.copy(path_or_folder_x,
            path_or_folder, 
            recursive = recursive, overwrite = F)
  return(path)
}) %>% unlist()


# file.copy("processed_data/HK5M2BBXY/bwa/mergedLibrary/ataqv/narrowPeak/html/",
#           "web/processed_data/HK5M2BBXY/bwa/mergedLibrary/ataqv/narrowPeak/html/", 
#           recursive = T)
```
